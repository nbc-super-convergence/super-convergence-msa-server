...
export const registerValidation = async (payload, loginId) => {
  ...
  if (error) {
    logger.info(
      `[ AUTH: registerRequestHandler ]  VALIDAION_ERROR  ===>>> ${error.details[0].message}`,
    );
  ...
  if (payload.password !== payload.passwordConfirm) {
    logger.info(
      `[ AUTH: registerRequestHandler ]  NOT_MATCH_PASSWORD_CONFIRM  ===>>> 패스워드 확일 불일치`,
    );
  ...
  // 이미 존재하는 아이디
  if (loginId) {
    logger.info(`[ AUTH: registerRequestHandler ]  ALREADY_EXIST_ID  ===>>> 이미 가입된 ID`);
    return config.FAIL_CODE.ALREADY_EXIST_ID;
  }
  ...
  if (existNick) {
    logger.info(
      `[ AUTH: registerRequestHandler ]  ALREADY_EXIST_NICKNAME  ===>>> 이미 가입된 NICK_NAME`,
    );
...
export const loginValidation = async (payload, loginId) => {
  ...
  if (error) {
    logger.info(
      `[ AUTH: loginRequestHandler ]  VALIDAION_ERROR  ===>>> ${error.details[0].message}`,
    );
  ...
  // DB 체크
  if (!loginId) {
    logger.info(`[ AUTH: loginRequestHandler ]  ID_OR_PASSWORD_MISS  ===>>> 존재하지 않는 ID`);
  ...
  if (!checkPassword) {
    logger.info(`[ AUTH: loginRequestHandler ]  ID_OR_PASSWORD_MISS  ===>>> 비밀번호 불일치`);
    return config.FAIL_CODE.ID_OR_PASSWORD_MISS;
  }
